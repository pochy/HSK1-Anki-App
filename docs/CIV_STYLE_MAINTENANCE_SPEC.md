...
interface BuildingMetadata {
  ...
  last_decay_time?: number; // 最終減衰時刻（テスト用、Unix timestamp）
}
```
...
function calculateCardContribution(card: Card, currentTime: number): number {
  // 基本貢献度
  let contribution = 50;
...
function updateCardStatus(card: Card, currentTime: number): void {
  const daysSinceReview = calculateDaysSinceReview(card);
  const expectedInterval = card.srs_data.interval;
...
function updateBuildingMetrics(building: Building, currentTime: number): void {
  // 維持レベルの更新
...
function applyUpgradeEffect(building: Building, effect: UpgradeEffect): void {
  switch (effect.type) {
...
テスト用に、維持レベルが時間経過で自動的に減衰する機能を実装しています。

```typescript
// 10秒ごとに1%減衰（テスト用）
const decayInterval = 10; // 秒
...
#### 5.2.2 維持レベルが0%になるまでの時間計算

施設詳細画面で、復習しない場合に維持レベルが0%になるまでの推定時間を表示します。

```typescript
function calculateDaysUntilZero(
  building: Building,
  cards: Card[],
  currentTime: number
): number | null {
...
アプリの設定画面から、以下の設定を変更できます：

- **新規カードの初期復習間隔**: 1-30日の範囲で設定可能（デフォルト: 1日）
- **減衰率の倍率**: 0.1-5.0倍の範囲で設定可能（デフォルト: 1.0倍）

設定はLocalStorageに自動保存され、次回起動時も保持されます。

#### 5.2.4 ヘルプモーダル（遊び方ガイド）

ゲーム画面の「？」ボタンから、遊び方ガイドを表示できます。以下の内容を含みます：
...
- ✅ 新規カードの初期復習間隔設定（1-30日）
- ✅ 減衰率の倍率設定（0.1-5.0倍）
- ✅ 設定のLocalStorage保存

### 9.4 テスト機能

- ✅ 時間ベースの減衰（10秒で1%減、テスト用）
...
5. **統計ダッシュボード**: 詳細な学習分析
6. **AR機能**: 現実世界に都市を表示
7. **施設のアップグレード**: リソースを使用した施設強化
8. **地区システム**: 同じカテゴリーの施設を近くに配置するボーナス
9. **施設の連鎖効果**: 特定の施設の組み合わせで特別な効果
10. **都市イベント**: 知識の祭典、技術革新、災害など
...
- **LocalStorageキー**:
  - `civ_maintenance_city`: 都市データ（JSON文字列）
  - `civ_maintenance_cards`: カードデータ（JSON文字列）

### 12.2 更新頻度

- **メトリクス更新**: 10秒ごと（`updateAllBuildingMetrics`）
- **時間ベース減衰**: 10秒ごとに1%減（テスト用）
- **LocalStorage保存**: ストア更新時に自動保存

### 12.3 カードの状態遷移

1. **active**: 復習から期待間隔の80%以内
2. **decaying**: 期待間隔の80%-120%
3. **inactive**: 期待間隔の120%-200%
4. **broken**: 期待間隔の200%以上

### 12.4 維持レベルの計算

維持レベルは、アクティブカードの平均貢献度を100で割った値です。
