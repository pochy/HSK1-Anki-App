...
interface SRSData {
  ...
  ease_factor: number; // 難易度係数（後方互換性のため、初期値: 2.5）
...
interface CardResult {
  ...
  new_ease_factor: number; // 新しい難易度係数（後方互換性のため）
...
- **品質スコア**: 各カードの評価と時間に基づくスコア（0-100）
- **レアリティ**: 品質スコアと連続正解数に基づくアイテムのレア度
- **連続正解ボーナス**: 連鎖が長いほど、より良いアイテムが作れる

### 6.3 プログレッション

- **記憶の安定性**: FSRSアルゴリズムによる記憶の定着度
- **復習間隔**: 学習状況に応じて自動調整される次回復習までの日数
- **統計情報**: 正答率、平均時間、最長ストリークなど（将来実装予定）
