<script lang="ts">
  ...
  onMount(() => {
    ...
    $learnedWords = allData.filter((item) =>
...
<div class="p-6 pb-24 flex flex-col items-center animate-slide-up">
  <div
    class="w-24 h-24 mb-6 rounded-3xl bg-gradient-to-br from-orange-400 to-red-500 shadow-xl flex items-center justify-center transform rotate-3"
  >
  ...
  <div class="w-full space-y-4">
    {#each levels as lvl}
      ...
      >
        <div
          class="{lvl.color} w-12 h-12 rounded-xl flex items-center justify-center font-bold text-xl mr-4 shrink-0"
        >
          {lvl.level}
        </div>
        <div class="flex-1">
          ...
          <div class="w-full bg-gray-100 h-1.5 rounded-full overflow-hidden">
            <div
              ...
              style="width: {getProgress(lvl.data)}%"
            ></div>
          </div>
        </div>
        <div class="ml-4 font-bold text-gray-300 text-sm">
          {getProgress(lvl.data)}%
        </div>
      </button>
    {/each}
  </div>
